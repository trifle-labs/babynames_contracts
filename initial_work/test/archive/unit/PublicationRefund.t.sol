...
contract PublicationRefundTest is TestHelpers {
    ...
    function test_SetPublicationTime() public {
        ...
        // Visible through getCategoryInfo for any 2025 category
    ...
    function test_SetPublicationTime_DifferentYears() public {
        // Create a 2024 category
        string[] memory names = _twoNames();
        market.createCategory(2024, 1, 0, BabyNameMarketCurve.Gender.Female, names, block.timestamp + 30 days, _emptyProofs());
    ...
    function test_RevertWhen_SetPublicationTime_AlreadySet() public {
        ...
        vm.expectRevert(BabyNameMarketCurve.PublicationTimeAlreadySet.selector);
        vm.prank(resolver);
        market.setPublicationTime(2025, block.timestamp - 2 hours);
    }

    function test_RevertWhen_SetPublicationTime_Zero() public {
        vm.expectRevert(BabyNameMarketCurve.InvalidPublicationTime.selector);
        vm.prank(resolver);
    ...
    function test_RefundInvalidBets_SingleUser() public {
        ...
        (, , , uint256 poolCollateral, ) = market.getPoolInfo(1);
        ...
        collateralAmts[0] = poolCollateral;
    ...
    function test_RefundInvalidBets_PartialRefund() public {
        // Buy loser pools first to build category collateral, avoiding pool-full
        _buyAs(bob, 2, 3e6);
        _buyAs(carol, 3, 3e6);

        // Alice buys twice — only refund the second purchase
        _buyAs(alice, 1, 3e6);
    ...
    function test_RefundInvalidBets_MultipleUsers() public {
        ...
        (uint256 aliceTokens, , ) = market.getUserPosition(1, alice);
        (, , , uint256 pool1Collateral, ) = market.getPoolInfo(1);
        (uint256 bobTokens, , ) = market.getUserPosition(2, bob);
        (, , , uint256 pool2Collateral, ) = market.getPoolInfo(2);
        ...
        users[0] = alice;
        tokenAmts[0] = aliceTokens;
        collateralAmts[0] = pool1Collateral;

        poolIds[1] = 2;
        users[1] = bob;
        tokenAmts[1] = bobTokens;
        collateralAmts[1] = pool2Collateral;
    ...
    function test_RefundThenResolve() public {
        // Alice bets before pub, Bob bets after pub (to be refunded)
        _buyAs(alice, 1, 5e6);
        _buyAs(bob, 2, 3e6);
        ...
        collateralAmts[0] = pool2Collateral;
    ...
    function test_RefundInvalidBets_EmitsEvent() public {
        ...
        vm.expectEmit(true, false, false, true);
        emit BetsRefunded(catId, poolCollateral, 1);
    ...
    function test_RefundAcrossCategories_SameYear() public {
        // Both categories are year 2025
        uint256 cat2 = _createTestCategoryMale();

        _buyAs(alice, 1, 3e6);  // cat1 pool
        _buyAs(bob, 4, 2e6);    // cat2 pool

        // One setPublicationTime covers both categories
        vm.startPrank(resolver);
        market.setPublicationTime(2025, block.timestamp - 1 hours);
    ...
    function test_RevertWhen_RefundNoPublicationTime() public {
        ...
        vm.prank(resolver);
        vm.expectRevert(BabyNameMarketCurve.PublicationTimeNotSet.selector);
        market.refundInvalidBets(catId, p, u, t, c);
    }
    ...
    function test_RevertWhen_RefundArrayLengthMismatch() public {
        ...
        vm.prank(resolver);
        vm.expectRevert(BabyNameMarketCurve.ArrayLengthMismatch.selector);
        market.refundInvalidBets(catId, p, u, t, c);
    }
    ...
    function test_RevertWhen_RefundExceedsBalance() public {
        ...
        vm.prank(resolver);
        vm.expectRevert(BabyNameMarketCurve.RefundExceedsBalance.selector);
        market.refundInvalidBets(catId, p, u, t, c);
    }
